...
var config = {
	...
	storagePath: path.join(__dirname, "../../cache", "storage.json")
};
...
getVertretungsplan(config.url, function (err, data) {
	...
	var storage = new PersistentStorage(config.storagePath);

	storage.set("latestPlanDate", data.date.getTime());

	storeResult(data, function (err) {
		if(err) {
